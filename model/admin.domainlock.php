...
class DomainLock
{
	...
	function listDomains($message="")
	{
		...
		$sql = "select distinct a.domain_id, a.domain_name, a.status, c.member_name from domain_lock a, domains b,
		        members c, products d where a.domain_id=b.domain_id and b.state=0 and b.member_id = c.member_id";
				
		if ($gtld)
		
			$sql .= " and a.domain_name like '%".$gtld."' ";
		//  $sql .= "and d.product_id=$gtld and d.domain_type=b.domain_type ";
		if ($member_name)
		    $sql .= " and c.member_name='".$member_name."' and c.member_id=b.member_id";
		if ($lock == 'locked' || $lock == 'unlocked')
		    $sql .= " and a.status='".$lock."' ";
		if ($domain)
		    $sql .= " and a.domain_name='".$domain."' ";
...
/*           echo  '<br>Query<br> <textarea style="height: 300px; width: 500 px; border: none; ">'.$xml.'</textarea><br>';  
	...
	*/
//        echo "<pre>XML: $xml <br><br> Res: $lockresult</pre>";
...
//		echo $sql;
		...
		//		echo "<br>query ".$sql;
         ...
         if ($lock_result != '')
            $lock_result = "Updated status of domain(s) ".$lock_result;
		 else 
