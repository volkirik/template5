...
class DomainManage
{
	function listDomain($message)
	{
		...
		$product_id	= $_REQUEST["product_id"];  //modified
		...
		if(DB_TYPE == "mysql")
		{
			...
			$sql = "select
					...
					domain_type,
					product_type,
					convert(varchar(20), add_date, 120),
					domain_year,
					domain_dns1,
					domain_dns2
				from	domains
		...
		if($searchDate <> 1)
		{
			if(DB_TYPE == "mysql")
			{
				$sql .= " and to_days(add_date) >= to_days('" . $startDate . "')" .
				 	" and to_days(add_date) <= to_days('" . $toDate . "')";
			}else {
				$sql .= " and add_date > '" . $startDate . "'" .
				 	" and add_date-1 < '" . $toDate . "'";
			}
		}
		if($domain_name != "")
		{
			$sql .= " and domain_name = '" . handleSQLData($domain_name) . "'";
		}
...
//		echo "<br>".$sql."<br>";
		...
		include(ROOT_DIR . "templates/" . CURRENT_SKIN . "/domain.domainlist.php");
	...
	function deleteDomain()
	{
		...
		$sql = "select
				domain_id, domain_name, domain_type, product_type, amount
    			from	domains
	    		where	domain_id = " . $domain_id . "
				and     state = 0
				and     member_id = " . $member_info[0];
		...
		$product_price = getProductPrice($gtld, $member_info[4], 2);
		$mode_id = getModeID($gtld, 2);
		
        $amt = checkBalance ($member_info[0], $product_price[1]);
        if ($amt == -1)
            $this->listDomain(MEMBER_0037. ' $'. $product_price[1] . ' required.');
        else {
            //echo " balance available &nbsp;". $member_info[0] ." &nbsp;". $product_price[1];
        }
		...
		$result = insertMoneyMode($member_info[0], 2, $mode_id, $product_price[1], $domain);
		if($result < 0)
		{
			$this->listDomain(DOMAIN_0073);
		}else {
			if(updateFunds($member_info[0], 2, $product_price[1]) < 0)
			{
				$this->listDomain(DOMAIN_0073);
			}
		}
      //////  refund fee
		...
		$result = insertMoneyMode($member_info[0], 1, $mode_id, $domain_price, $domain);
		if($result < 0)
		{
			$this->listDomain(DOMAIN_0073);
		}else {
			if(updateFunds($member_info[0], 1, $domain_price) < 0)
			{
				$this->listDomain(DOMAIN_0073);
			}
		}
	...
	function renewDomain()
	{
		...
		$product_price = getProductPrice($gtld, $member_info[4], 3);
		...
		$result = insertMoneyMode($member_info[0], 2, $mode_id, $product_price[$year], $domain);
		if($result < 0)
		{
