...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        $search = '/' . $ld . 'php.*?' . $rd . '/s'; // eski $search'e eşdeğer

        $source_content = preg_replace_callback($search, function ($matches) use ($ld, $rd) {
                $lineCount = substr_count($matches[0], "\n");
...
$text_blocks = preg_split("~{$ldq}.*?{$rdq}~s", $source_content ?? '');
        ...
        $timestamp = time();
        // Türkçe tarih istiyorsan locale ayarla

        $template_header = "<?php /* Smarty version ".$this->_version.", created on ".date("Y-m-d H:i:s", $timestamp)."\n";
