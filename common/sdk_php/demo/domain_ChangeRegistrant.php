...
$params = array(
	...
	'domain'     => $domain,
	'name'       => 'new registrant',//input new name
	//'regtype'    => 'In',//Special for EU: In- Individual, Co-Corporation.
	//'org'        => '',//especially for EU trade
	//'email'      => '',//especially for EU trade
);
$cmd        = $api->buildCommand('domain', 'ChangeRegistrant', $params);
$rs         = $api->request($cmd);
$lastResult = $api->getLastResult();
if ($rs) {
	$error = getError(__LINE__, $rs, $lastResult);
