<script language="javascript">
    function renewDomain(myForm) {
        var myYear = prompt("Bu alan adını kaç yıl uzatmak istersiniz?", '1');
                                    ...
                                    <td align="center">
                                        ...
                                        <select name="startYear">
                                            ...
                                            {section name=yr loop=$iyr}
                                                {if $startYear == $iyr[yr]}
                                                    <option value="{$iyr[yr]}" selected>{$iyr[yr]}</option>
                                                {else}
                                                    <option value="{$iyr[yr]}">{$iyr[yr]}</option>
                                                {/if}
                                            {/section}
                                        </select>
                                        <select name="startMonth">
                                            ...
                                            {section name=month loop=$imonth}
                                                {if $startMonth == $imonth[month]}
                                                    <option value="{$imonth[month]}" selected>{$imonth[month]}</option>
                                                {else}
                                                    <option value="{$imonth[month]}">{$imonth[month]}</option>
                                                {/if}
                                            {/section}
                                        </select>
                                        <select name="startDay">
                                            ...
                                            {section name=day loop=$iday}
                                                {if $startDay == $iday[day]}
                                                    <option value="{$iday[day]}" selected>{$iday[day]}</option>
                                                {else}
                                                    <option value="{$iday[day]}">{$iday[day]}</option>
                                                {/if}
                                            {/section}
                                        </select>
                                        ile 
            ...
            <form method="post" action="{$php_self}" name="domainManage" style="margin:0px">
                <table width="100%" border="0" cellspacing="1" cellpadding="2" align="center">
                    <tr class="fieldName"> 
                        ...
                        <td width="26%" height="20"><b><font color="#FFFFFF">Alan Adı (Kullanıcı Adı)</font></b></td>
                        <td width="20%" height="20"><b><font color="#FFFFFF">Dns1</font></b></td>
                        <td width="20%" height="20"><b><font color="#FFFFFF">Dns2</font></b></td>
                        <td width="20%" height="20"><b><font color="#FFFFFF">Kayıt Tarihi</font></b></td>
                        <td width="16%" height="20"><b><font color="#FFFFFF">Kayıt Yılı</font></b></td>
                    </tr>
                    {section name=i loop=$rs}
                        <tr> 
                            ...
                            <td height="20" class="p8" {$rs[i][1]} width="27%">
                                <a href="{$php_self}?action=showContact&domain_id={$rs[i][0][0]}" class="menu2"> 
                                    {$rs[i][0][1]}({$rs[i][0][8]})
                                </a>
                            </td>
                            <td height="20" class="p8" {$rs[i][1]} width="15%">
                                <a href="{$php_self}?action=showNS&domain_id={$rs[i][0][0]}" class="menu2"> 
                                    {$rs[i][0][6]}
                                </a>
                            </td>
                    ...
                    <tr align="center">
                        <td height="20" colspan="6">
                            <p>&nbsp;</p>
                            <p>
                                <input type="button" name="renew" value="Alan Adını Yenile" onClick="renewDomain(this.form);">
                                <input type="button" name="delete" value="Alan Adını Sil" onClick="deleteDomain(this.form);">
                            </p>
                        </td>
                    </tr>
                </table>
